...
namespace Eventos_S4_Tr1
{
    public partial class frm_Eventos : Form
    {
        public frm_Eventos()
        {
            ...
            /*Usamos '+=' para agregar/suscribir el método al evento DragEnter del panel.
            Este operador sirve para suscribirse a eventos.
            Ejemplo rápido con lambda:
            pnl_Evento_DD.DragEnter += (s, e) => { e.Effect = DragDropEffects.Copy; };*/
            pnl_Evento_DD.DragEnter += pnl_Evento_DD_DragEnter;
        }

        /*Este evento se dispara cuando algo arrastrado entra al panel.
        Aquí indicamos si el panel acepta lo que se está arrastrando mediante 'e.Effect'.
        Si no se establece, el sistema no permite soltar el archivo.*/
        private void pnl_Evento_DD_DragEnter(object sender, DragEventArgs e)
        {
            ...
            // Verifica si se está arrastrando un Bitmap directamente
            ...
            // Si no es un formato soportado, no permitir soltar
            ...
            try
            {
                ...
                if (e.Data != null && e.Data.GetDataPresent(DataFormats.FileDrop))
                {
                    ...
                    if (archivos.Length > 0)
                    {
                        ...
                        if (ext != ".jpg" && ext != ".jpeg" && ext != ".png" && ext != ".bmp" && ext != ".gif")
                        {
                            MessageBox.Show("Formato no soportado. Selecciona un archivo de imagen.", "Formato no soportado");
                            return;
                        }

                        // Se Clona la imagen en memoria para usar
                ...
                // Si vino un Bitmap directamente
                if (e.Data != null && e.Data.GetDataPresent(DataFormats.Bitmap))
                {
        ...
        private void txt_Evento_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            // Aquí validamos que solo se permitan caracteres numéricos y controles
        ...
        {
            // Ejemplo de suscripción con lambda (en lugar de un método aparte):
            // btn_Timer.Click += (s, ev) => Timer.Start();
            // Aquí preferimos usar el método btn_Timer_Click
            Timer.Start();
        }

        private void lbl_evento_Click(object sender, EventArgs e)
        {
            // El Click del label está suscrito desde el diseñador: lbl_evento.Click += lbl_evento_Click;
        ...
        {
            // Timer.Tick se suscribe en el diseñador: Timer.Tick += Timer_Tick;
            // También se establece el Interval en 1000 ms
            segundos++;
            if (segundos >= 60)
            {
                segundos = 0;
